...
const Navbar = () => {
  ...
  return (
    ...
    <SearchIcon className='mx-md:hidden w-6 h-6 cursor-pointer'/>
    {
      !user ? (
          <button onClick={openSignIn} className='px-4 py-1 sm:px-7 sm:py-2 bg-red-500 
             hover:bg-red-600 transition rounded-full font-medium cursor-pointer'> Login </button>
    ...
    <MenuIcon className='max-md:ml-4 md:hidden w-8 h-4 cursor-pointer' onDoubleClick={()=>
     setIsOpen(!isOpen)} />
    </div>
  )
}
